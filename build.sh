...
if [ ! -d "drivers/kernelsu" ]; then
    ...
    if [ "$DRY_RUN" -eq 0 ]; then
        ...
        echo "[DRY-RUN] Intégration KernelSU simulée"
    fi
fi
...
EOF
        ...
        # ==================== CORRECTION ROBUSTE DE fs/open.c ====================
        echo "=== Correction robuste de fs/open.c ==="
        
        # Créer une copie de sauvegarde
        cp fs/open.c fs/open.c.bak
        
        # Utiliser perl pour une correction plus fiable
        perl -i -pe '
            # État: 0 = recherche, 1 = dans do_faccessat
            ...
            # Si on est dans do_faccessat
            if ($in_faccessat) {
                # Supprimer tout bloc SUSFS mal placé
                ...
                # Vérifier si le hook KSU est déjà présent
                if (/^\s*ksu_handle_faccessat/) {
                    $found_ksu = 1;
                }
                
                # Après les déclarations de variables, avant le code d'exécution
                if (/^\s*unsigned int lookup_flags = LOOKUP_FOLLOW;/) {
                    # Ajouter le bloc SUSFS après la déclaration
                    if (!$found_correct) {
                        $found_correct = 1;
                        # Ajouter le bloc après cette ligne
            ...
            # Supprimer les blocs KSU_SUSFS_SUS_PATH en dehors de do_faccessat
            if (!$in_faccessat) {
        ...
        if [ ! -s "fs/open.c" ]; then
            echo "⚠️ fs/open.c est vide, restauration de la sauvegarde..."
            cp fs/open.c.bak fs/open.c
            # Correction manuelle simplifiée
            sed -i '/#ifdef CONFIG_KSU_SUSFS_SUS_PATH/,/#endif/d' fs/open.c
    ...
    if (status) {\
        ...
        echo "✅ fs/open.c corrigé"

        # ==================== CORRECTION DE fs/proc/task_mmu.c ====================
        if [ -f "fs/proc/task_mmu.c.rej" ]; then
            echo "=== Correction de fs/proc/task_mmu.c ==="
            sed -i '/#ifdef CONFIG_KSU_SUSFS_SUS_MAPS/,/#endif/d' fs/proc/task_mmu.c
...
orig_flow:\
            ...
            echo "✅ fs/proc/task_mmu.c corrigé"
        fi

        # Supprimer les autres .rej
        find . -name "*.rej" -delete 2>/dev/null
