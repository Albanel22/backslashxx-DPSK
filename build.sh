#!/bin/bash
set -e
echo "=== BUILD WINNER : KernelSU v3.2.5-76+ (0b138d6a) + SuSFS + fix UAPI + sys_reboot + PANEL_NOTIFICATIONS ==="
df -h
...
# Nettoyage obligatoire pour éviter l'erreur "is not clean"
make mrproper || true
rm -rf out
mkdir -p out
...
echo "=== Vérification options importantes ==="
grep -E "CONFIG_KSU|CONFIG_PANEL_NOTIFICATIONS" out/.config | head -20
...
if [ -f out/.config ]; then
    if grep -q "CONFIG_PANEL_NOTIFICATIONS=y" out/.config; then
        ...
        echo "⚠️  Activation forcée de CONFIG_PANEL_NOTIFICATIONS..."
        ./scripts/config --file out/.config --enable PANEL_NOTIFICATIONS
        echo "CONFIG_PANEL_NOTIFICATIONS=y" >> out/.config
    fi
else
    echo "❌ out/.config introuvable !"
    exit 1
fi

grep "PANEL_NOTIFICATIONS" out/.config || echo "⚠️  Toujours pas trouvé"
