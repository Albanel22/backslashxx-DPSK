...
echo "=== BUILD ULTIME : KernelSU + SuSFS + FocalTech (0flash_mmi) ==="
df -h
...
if [ -f "/tmp/KernelSU/uapi/ksu.h" ]; then
    sed -i 's/#define KERNEL_SU_VERSION KERNEL_SU_VERSION/#define KERNEL_SU_VERSION 32601/' /tmp/KernelSU/uapi/ksu.h 2>/dev/null || true
fi
...

{
    ...
    echo "CONFIG_TOUCHSCREEN_FOCALTECH_0FLASH=y"
    echo "CONFIG_TOUCHSCREEN_FOCALTECH_0FLASH_MMI=y"
} >> out/.config
